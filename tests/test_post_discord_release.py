...
The script used to fail outright when a changelog section did not fit one
Discord embed description, which fails the release's Discord step while every
other release step has already succeeded. That happened once on the
advanced-power-infrastructure repository: a section of 8213 characters against
a 4084 character limit, because that release carried two releases' worth of
work.

No section in this repository has come close yet, but any repository can end up
in that position, so the splitting is here rather than waiting for it to fail.
These cover that splitting.
"""
...
class SplitSectionTests(unittest.TestCase):
    ...
    def test_every_real_changelog_section_can_be_posted(self):
        ...
        branch changes src/changelog.txt, so asserting on the size of any
        particular section would make this suite pass or fail depending on
        which branch it runs from. What matters is that whatever is there
        splits correctly.
        """
...
class PayloadTests(unittest.TestCase):
    class Args:
        ...
        mod_title = "Advanced Fluid Infrastructure"
