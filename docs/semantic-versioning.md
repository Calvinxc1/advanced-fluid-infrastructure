...
The current release automation creates a Gitea release only when a merge to `main` changes `src/info.json` version compared to the previous `main` commit. A version bump is therefore the deployment trigger.
